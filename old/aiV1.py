...
def evaluate_configuration_batch(meta, move_sets_batch,
                                 ...
                                 scaler, encoder, cat_feature_names, model):
...
def train_model_balanced():
    feature_dicts = []
    targets = []
    missing = set()
    for moveset in smogon_data:
        name = moveset.get("pokemonName", "")
        pinfo = pokemon_lookup.get(name.lower())
        if pinfo is None:
            missing.add(name)
            continue
        feats = extract_features_balanced(moveset, pinfo)
        feature_dicts.append(feats)
        targets.append(feats["heuristic_score"])
    # Export missing names to file for alias mapping
    if missing:
        alias_file = DATA_DIR / "missing_pokemon_names.txt"
        with open(alias_file, "w", encoding="utf-8") as f:
            for nm in sorted(missing):
                f.write(nm + "\n")
        print(f"Exported {len(missing)} missing Pokémon names to {alias_file} for alias input.")

    # Build DataFrame
    df = pd.DataFrame(feature_dicts)
    y = df["heuristic_score"]
    df_features = df.drop(columns=["heuristic_score"])

    # Define numeric and categorical feature columns.
    numeric_cols = ['HP','Attack','Defense','SpecialAttack','SpecialDefense','Speed',
    ...
    cat_mat = encoder.fit_transform(df_features[categorical_cols])
    cat_names = encoder.get_feature_names_out(categorical_cols)
    df_cat = pd.DataFrame(cat_mat, columns=cat_names, index=df_features.index)

    # Scale numeric features.
    scaler = StandardScaler()
    num_mat = scaler.fit_transform(df_features[numeric_cols])
    df_num = pd.DataFrame(num_mat, columns=numeric_cols, index=df_features.index)

    # Combine numeric and categorical features.
    X = pd.concat([df_num, df_cat], axis=1)
    print(f"Training on full dataset: samples={X.shape[0]}, features={X.shape[1]}")

    # Train XGBoost on entire data
    model = XGBRegressor(objective='reg:squarederror', n_estimators=100, random_state=42)
    model.fit(X, y)
    preds = model.predict(X)
    mse = mean_squared_error(y, preds)
    print(f"Model trained on full data. In-sample MSE: {mse:.4f}")

    return model, scaler, encoder, cat_names, numeric_cols, categorical_cols

# Execute training
model, scaler, encoder, cat_feature_names, numeric_cols, categorical_cols = train_model_balanced()
    ...
    for meta in metas:
        results = Parallel(n_jobs=-1, verbose=0)(
            delayed(evaluate_configuration_batch)(
                ...
                scaler, encoder, cat_feature_names,
                model
            )
...
def run_reinforcement_learning(num_rounds):
    """For each round, generate a best moveset for every Pokémon,
    score it via GPT, then re‑fit the XGB model on those GPT scores."""
    ...
    for r in range(1, num_rounds+1):
        ...
        # Assemble training data & retrain
        df_rl = pd.DataFrame(feats_list)
        y_rl  = np.array(scores)
        X_num = scaler.transform(df_rl[numeric_cols])
        X_cat = encoder.transform(df_rl[categorical_cols])
        X_rl  = np.hstack([X_num, X_cat])
        model.fit(X_rl, y_rl, xgb_model=None)  # reset to use only GPT rewards
        print(f"Completed round {r}, retrained model on {len(scores)} samples.")

    # Persist the final model + preprocessors
    joblib.dump(
        (model, scaler, encoder, cat_feature_names, numeric_cols, categorical_cols),
        "moveset_rl_model.pkl"
    )
    print("Reinforcement‑trained model saved to moveset_rl_model.pkl")
...
def main():
    ...
    # If user wants to resume from an existing RL model, load it first
    if args.load_model:
        m, s, e, cfn, ncol, ccol = joblib.load(args.load_model)
        globals().update({
            "model":            m,
            "scaler":           s,
            "encoder":          e,
            "cat_feature_names":cfn,
            "numeric_cols":     ncol,
            "categorical_cols": ccol,
        })
        print(f"Resuming RL training from '{args.load_model}'")
    ...
    if args.rounds > 0:
        run_reinforcement_learning(args.rounds)
        if args.save_model:
            joblib.dump(
                (model, scaler, encoder, cat_feature_names, numeric_cols, categorical_cols),
                "moveset_rl_model.pkl"
            )
            print("Saved RL‑trained model to moveset_rl_model.pkl")

    if args.pokemon:
        # Load the RL‑trained model + preprocessors
        loaded = joblib.load("moveset_rl_model_r1.pkl")
        # Overwrite the module‑level globals so all functions use the RL model
        globals().update({
            "model":            loaded[0],
            "scaler":           loaded[1],
            "encoder":          loaded[2],
            "cat_feature_names":loaded[3],
            "numeric_cols":     loaded[4],
            "categorical_cols": loaded[5],
        })
