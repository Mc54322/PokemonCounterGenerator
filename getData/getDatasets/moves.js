...
exports.Moves = {
        ...
        onHit: function (target, source) {
            ...
            for (var _i = 0, _a = this.dex.types.names(); _i < _a.length; _i++) {
                ...
                if (source.hasType(typeName))
                    continue;
                var typeCheck = this.dex.types.get(typeName).damageTaken[attackType];
    ...
    hardpress: {
        ...
        basePowerCallback: function (pokemon, target) {
            var hp = target.hp;
            var maxHP = target.maxhp;
            var bp = Math.floor(Math.floor((100 * (100 * Math.floor(hp * 4096 / maxHP)) + 2048 - 1) / 4096) / 100) || 1;
            this.debug('BP for ' + hp + '/' + maxHP + " HP: " + bp);
            return bp;
        },
        category: "Physical",
    ...
    healblock: {
        ...
        condition: {
            duration: 5,
            durationCallback: function (target, source, effect) {
                if ((effect === null || effect === void 0 ? void 0 : effect.name) === "Psychic Noise") {
                    return 2;
                }
                if (source === null || source === void 0 ? void 0 : source.hasAbility('persistent')) {
    ...
    icespinner: {
        ...
        onAfterSubDamage: function (damage, target, source) {
            if (source.hp) {
                this.field.clearTerrain();
            }
        },
    ...
    metronome: {
        ...
        flags: { failencore: 1, nosleeptalk: 1, noassist: 1, failcopycat: 1, failmimic: 1, failinstruct: 1 },
        onHit: function (pokemon) {
            var moves = this.dex.moves.all().filter(function (move) { return ((!move.isNonstandard || move.isNonstandard === 'Unobtainable') &&
    ...
    mirrormove: {
        ...
        onTryHit: function (target, pokemon) {
            ...
            this.actions.useMove(move.id, pokemon, { target: target });
            return null;
        },
        callsMove: true,
        secondary: null,
    ...
    swallow: {
        ...
        onHit: function (pokemon) {
            var _a;
            var layers = ((_a = pokemon.volatiles['stockpile']) === null || _a === void 0 ? void 0 : _a.layers) || 1;
            var healAmount = [0.25, 0.5, 1];
            var success = !!this.heal(this.modify(pokemon.maxhp, healAmount[layers - 1]));
            if (!success)
                this.add('-fail', pokemon, 'heal');
            pokemon.removeVolatile('stockpile');
        ...
        condition: {
            onStart: function (pokemon, source) {
                ...
                if (this.effectState.startingTurn === 255) {
                    this.hint("In Gen 8+, Wish will never resolve when used on the ".concat(this.turn, "th turn."));
                }
            },
            onResidualOrder: 4,
            onResidual: function (target) {
